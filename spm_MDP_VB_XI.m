function [MDP] = spm_MDP_VB_XI(MDP,OPTIONS)

% active inference and learning using variational message passing
% FORMAT [MDP] = spm_MDP_VB_XI(MDP,OPTIONS)
%  (copy of spm_MDP_VB_X_tutorial(MDP,OPTIONS) )
...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)                  % number of MDPs
        ...
        %------------------------------------------------------------------
        OUT(:,i) = spm_MDP_VB_XI(MDP(:,i),OPTIONS);
                    ...
                    for j = 1:S         % loop over future time points
                        ...
                        for f = 1:Nf(m)
                            ...
                            if dF > exp(-8) || i > 4
                                ...
                                % if t==2 && j==2;  warning('debug stop'); end
                            else
                                F(k) = G(k);
                            end
